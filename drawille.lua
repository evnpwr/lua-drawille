...
function Canvas.set(self, x, y, r, g, b, a, overwrite)
    local row = math.floor(y / 4)
    local col = math.floor(x / 2)
    if self.pixel_matrix[row] == nil then
    ...
    if overwrite then
        if (pixel.r == r and
            ...
            pixel.b == b and
            pixel.a == a) then
            pixel.braille_rep = bor(pixel.braille_rep,
                                    pixel_map[band(y,3) + 1][band(x,1) + 1])
        else
            pixel.braille_rep = pixel_map[band(y,3) + 1][band(x,1) + 1]
        end
    else
        pixel.braille_rep = bor(pixel.braille_rep,
                                pixel_map[band(y,3) + 1][band(x,1) + 1])
